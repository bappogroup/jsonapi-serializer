...
module.exports = function (collectionName, record, payload, opts) {
  ...
  function getRefType(str, attrVal, refOpts) {
    if (isFunction(refOpts._type)) {
      return refOpts._type(str, attrVal || {});
    } else if (refOpts._type) {
      return refOpts._type;
    }
    return getType(str, attrVal);
  }
  ...
  this.serializeRef = function (dest, current, attribute, opts) {
    ...
    var type = getRefType(attribute, dest, opts);
