...
module.exports = function (collectionName, record, payload, opts) {
  ...
  function getType(str, attrVal, refOpts = opts) {
    var type;
    attrVal = attrVal || {};

    if (isFunction(refOpts._typeForAttribute)) {
      type = refOpts._typeForAttribute(str, attrVal);
    }

    // If the pluralize option is on, typeForAttribute returned undefined or wasn't used
    if ((refOpts._pluralizeType === undefined || refOpts._pluralizeType) && type === undefined) {
